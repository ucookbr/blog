...
class iworks_options {
	...
	public function build_options( $option_group = 'index', $echo = true, $term_id = false ) {
		...
		if ( ! is_array( $options['options'] ) ) {
			echo '<div class="below-h2 error"><p><strong>'.__( 'An error occurred while getting the configuration.', 'reading-position-indicator' ).'</strong></p></div>';
			return;
		}
			...
			switch ( $option['type'] ) {
				...
				case 'image':
					...
					$content .= sprintf(
						...
						__( 'Upload image', 'reading-position-indicator' ),
						$html_element_name
					);
					if ( ! empty( $value ) || ( array_key_exists( 'default', $option ) && $value != $option['default'] ) ) {
						$content .= sprintf(
							...
							__( 'Delete image', 'reading-position-indicator' ),
							$html_element_name
						);
					}
				break;
...
<?php
	...
	public function get_switch_button_data() {
		$data = array(
			'labels' => array(
				'off_label' => __( 'OFF', 'reading-position-indicator' ),
				'on_label' => __( 'ON', 'reading-position-indicator' ),
			),
		);
		return $data;
	}
