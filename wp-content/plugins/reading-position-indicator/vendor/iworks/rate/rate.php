...
if ( ! class_exists( 'iworks_rate' ) ) {
	class iworks_rate {
		...
		protected function render_rate_message( $plugin ) {
			...
			$msg = __( "Hey %s, you've been using %s for a while now, and we hope you're happy with it.", 'reading-position-indicator' ) . '<br />'. __( "We've spent countless hours developing this free plugin for you, and we would really appreciate it if you dropped us a quick rating!", 'reading-position-indicator' );
			...
			<div class="iworks-notice-logo"><span></span></div>
				...
				<div class="iworks-notice-cta">
					<button class="iworks-notice-act button-primary" data-msg="<?php _e( 'Thanks :)', 'reading-position-indicator' ); ?>">
						<?php
						printf(
							__( 'Rate %s', 'reading-position-indicator' ),
							esc_html( $plugin->title )
						); ?>
					</button>
					<button class="iworks-notice-dismiss" data-msg="<?php _e( 'Saving', 'reading-position-indicator' ); ?>">
						<?php _e( 'No thanks', 'reading-position-indicator' ); ?>
					</button>
				</div>
			<?php
		}
